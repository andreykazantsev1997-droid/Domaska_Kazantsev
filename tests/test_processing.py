...
@pytest.fixture
def get_test_state() -> list[dict[str, Any]]:
    return [
...
def test_filter_by_state(get_test_state: list) -> None:
...
def test_filter_by_state_canceled(get_test_state: list[dict[str, Any]]) -> None:
    result = filter_by_state(get_test_state, key="CANCELED")
    assert len(result) == 1
    assert result[0]["id"] == 594226727


def test_filter_by_state_empty(get_test_state: list[dict[str, Any]]) -> None:
    assert filter_by_state([], "EXECUTED") == []


@pytest.fixture
def sample_data() -> list[dict[str, Any]]:
    return [{"date": "2019-10-10", "id": 1}, {"date": "2023-01-01", "id": 2}, {"date": "2021-05-20", "id": 3}]
...
def test_sort_by_date_empty(sample_data: list[dict[str, Any]]) -> None:
    assert sort_by_date([]) == []
